...
wpa_cli_path = '/usr/bin/wpa_cli'

class wpa_cli:
    def __init__(self, interface=None):
        self.interface = interface
        self.wpa_status = self.status()
        if self.wpa_status[0] <> 0:
            fail('Failed to connect wpa daemon')

    def run_cmd(self, args):
        if self.interface:
            return capture('%s -i%s %s' % (wpa_cli_path, self.interface, args))
        else:
            return capture('%s %s' % (wpa_cli_path, args))
    ...
    def scan_results(self):
        return self.run_cmd('scan_results')[1][2:]

    ''' Get WPA connection status '''
    def status(self):
    ...
    def add_network(self):
        if self.interface:
            return self.run_cmd('add_network')[1][0].strip()
        return self.run_cmd('add_network')[1][1].strip()

    ''' Remove network connection '''
    def remove_network(self, network_id):
        return self.run_cmd('remove_network %s' % network_id)

    ''' List network connection '''
    def list_networks(self):
        return self.run_cmd('list_networks')[1][2:]

    ''' Select network connection '''
    def select_network(self, network_id):
        return self.run_cmd('select_network %s' % network_id)

    def enable_network(self, network_id):
        return self.run_cmd('enable_network %s' % network_id)

    def disable_network(self, network_id):
        return self.run_cmd('disable_network %s' % network_id)

    ''' Set network connection variables '''
    def set_network(self, network_id, variable, value):
        return self.run_cmd('set_network %s %s %s' % (network_id, variable, value))

    ''' WPA-PSK authentication '''
    def authenticate_psk(self, network_id, ssid, psk):
        self.set_network(network_id, 'key_mgmt', 'WPA-PSK')
        self.set_network(network_id, 'ssid', r'\"%s\"' % ssid)
        self.set_network(network_id, 'psk', r'\"%s\"' % psk)
        self.select_network(network_id)

    ''' PEAP authentication '''
    def authenticate_peap(self, network_id, ssid, identity, password):
        self.set_network(network_id, 'key_mgmt', 'WPA-EAP')
        self.set_network(network_id, 'eap', 'PEAP')
        self.set_network(network_id, 'ssid', r'\"%s\"' % ssid)
        self.set_network(network_id, 'identity', r'\"%s\"' % identity)
        self.set_network(network_id, 'password', r'\"%s\"' % password)
        self.set_network(network_id, 'phase2', r'\"auth=MSCHAPV2\"')
        self.select_network(network_id)

    ''' Reassociate connection settings '''
    def reassociate(self):
        return self.run_cmd('reassociate')

    ''' Get connected interface '''
    def get_interface(self):
        if self.interface:
            return self.run_cmd('interface')[1][1].strip()
        return self.run_cmd('interface')[1][2].strip()

    ''' Disconnect connection '''
    def disconnect(self):
        return self.run_cmd('disconnect')
...
class Dev:
    ...
    def up(self):
        wpa = wpa_cli(self.dev)
        wpa.disconnect()

        ''' Seems peap auth. '''
        if self.user and self.user != "" and self.password and self.password != "":
            network_id = wpa.add_network()
            wpa.authenticate_peap(network_id, self.remote, self.user, self.password)
            wpa.reassociate()
        elif self.password and self.password != "":    # Seems wpa-psk auth.
            network_id = wpa.add_network()
            wpa.authenticate_psk(network_id, self.remote, self.password)
            wpa.reassociate()
        else:
            fail("Password must have at least eight characters")
...
def modes():
    return "device,remote,scan,net,auto,passauth,loginauth"
...
def getAuthentication(name=None):
    dev = Dev(name)
    ''' Seems peap auth. '''
    if dev.user and dev.user != "" and dev.password and dev.password != "":
        return "%s\nloginauth\n%s\n%s" % (name, dev.user, dev.password)
    elif dev.password and dev.password != "":    # Seems wpa-psk ath.
        return "%s\npassauth\n%s" % (name, dev.password)
    return "%s\nnone" % name
